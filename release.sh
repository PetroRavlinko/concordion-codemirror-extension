...
git push --force --quiet "https://${GIT_TOKEN}@github.com/${GIT_USER_ACCOUNT}/concordion-codemirror-extension.git" master --tags > /dev/null 2>&1
...
curl -u "${GIT_USER_ACCOUNT}:${GIT_PASSWORD}" -H "Content-Type: application/json" -X POST -d "${release_json}" https://api.github.com/repos/"${GIT_USER_ACCOUNT}"/concordion-codemirror-extension/releases
...
git push --force --quiet "https://${GIT_TOKEN}@github.com/${GIT_USER_ACCOUNT}/concordion-codemirror-extension.git" ${version} > /dev/null 2>&1
...
git push --force --quiet "https://${GIT_TOKEN}@github.com/${GIT_USER_ACCOUNT}/concordion-codemirror-extension.git" master > /dev/null 2>&1
